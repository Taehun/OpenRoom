...
const PENDING_CUTOUT_PRODUCT_IDS: readonly string[] = [
  // Every catalog product has a cutout today; a newly added product lands here
  // until `pnpm assets:products` generates its image.
];
...
describe("photo assets", () => {
  ...
  it("has at least five stable products for every category", () => {
    ...
    // Products without a cutout are listed above; the list is empty once the
    // pipeline has run for every product.
    expect(
