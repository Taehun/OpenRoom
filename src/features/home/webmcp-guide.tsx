...
function statusMessage(status: CompatibilityStatus | null): string {
  ...
  switch (status.kind) {
    ...
    case "insecure-context":
      return "WebMCP is only exposed on secure pages; open Nook over HTTPS or on localhost.";
  }
}
...
export function WebMcpGuide({ onCheckAgain, status }: WebMcpGuideProps) {
  return (
    <main className={styles.guide}>
      <section className={styles.hero} aria-labelledby="nook-heading">
        <div className={styles.heroCopy}>
          ...
          <p className={styles.intro}>
            {"Nook turns a room photo into a storefront that both people and AI agents can edit. This browser has not exposed WebMCP yet, so here is how to get it running, and how to try the demo without it."}
          </p>
        </div>
      ...
      <section className={styles.panel} aria-labelledby="webmcp-source-heading">
        ...
        <p>
          {"Nook is MIT-licensed. The source in this repository includes the deterministic solver, the photo compositor, and the WebMCP tools."}
        </p>
      </section>
    </main>
  );
